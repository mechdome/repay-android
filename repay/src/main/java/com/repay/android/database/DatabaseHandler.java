...
public class DatabaseHandler extends SQLiteOpenHelper
{
	...
	public void onCreate(SQLiteDatabase db)
	{
		...
		db.execSQL(String.format("CREATE TABLE %s (%s TEXT PRIMARY KEY, %s TEXT, %s TEXT, %s TEXT)", Names.F_TABLENAME, Names.F_REPAYID, Names.F_LOOKUPURI, Names.F_NAME, Names.F_DEBT));

		// Create Debts table
		db.execSQL(String.format("CREATE TABLE %s (%s INTEGER PRIMARY KEY AUTOINCREMENT, %s TEXT NOT NULL, " + "%s TEXT, %s TEXT, %s TEXT, FOREIGN KEY(%s) REFERENCES %s(%s))", Names.D_TABLENAME, Names.D_DEBTID, Names.D_REPAYID, Names.D_DATE, Names.D_AMOUNT, Names.D_DESCRIPTION, Names.D_REPAYID, Names.F_TABLENAME, Names.F_REPAYID));
	}
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion)
	{
		if (oldVersion == 1 && newVersion == 2)
		{
			...
			try
			{
				...
				c = db.query(Names.D_TABLENAME, new String[]{Names.D_REPAYID, Names.D_DATE, Names.D_AMOUNT, Names.D_DESCRIPTION}, null, null, null, null, null);
				c.moveToFirst();
	...
	public void updateDebt(Debt debt) throws SQLException, NullPointerException
	{
		...
		SQLiteDatabase db = this.getWritableDatabase();
		db.update(Names.D_TABLENAME, values, Names.D_DEBTID + "=?", new String[]{Integer.toString(debt.getDebtID())});
		db.close();
	}
	 ...
	 */
	public ArrayList<Debt> getDebtsByRepayID(final String repayID) throws SQLException, ParseException, CursorIndexOutOfBoundsException, NullPointerException
	{
		ArrayList<Debt> debts = null;
		SQLiteDatabase db = this.getReadableDatabase();
		Cursor c = db.query(Names.D_TABLENAME, new String[]{Names.D_DEBTID, Names.D_DATE, Names.D_AMOUNT, Names.D_DESCRIPTION}, Names.D_REPAYID + "=?", new String[]{repayID}, null, null, null);
	 ...
	 * @throws NullPointerException
	 */
	public void removeDebt(int debtID) throws SQLException, IndexOutOfBoundsException, NullPointerException
	{
		SQLiteDatabase db = this.getWritableDatabase();
		db.delete(Names.D_TABLENAME, Names.D_DEBTID + "=?", new String[]{Integer.toString(debtID)});
		db.close();
	}
	...
	public Debt getMostRecentDebt() throws ParseException, NullPointerException, SQLiteException, CursorIndexOutOfBoundsException
	{
		...
		SimpleDateFormat sdf = new SimpleDateFormat(DATE_FORMAT);
		db.close();
		return new Debt(c.getInt(0), c.getString(1), sdf.parse(c.getString(2)), new BigDecimal(c.getString(2)), c.getString(3));
	}
	...
	public Debt getDebtByIDs(int debtID) throws SQLException, IndexOutOfBoundsException, ParseException
	{
		...
		Debt debt = null;
		SQLiteDatabase db = this.getReadableDatabase();
		c = db.query(Names.D_TABLENAME, new String[]{Names.D_DEBTID, Names.D_REPAYID, Names.D_DATE, Names.D_AMOUNT, Names.D_DESCRIPTION}, Names.D_DEBTID + "=?", new String[]{Integer.toString(debtID)}, null, null, null);
		if (c != null)
		{
			c.moveToFirst();
			SimpleDateFormat sdf = new SimpleDateFormat(DATE_FORMAT);
